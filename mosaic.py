...
def main(arguments = None):
    ...
    if not os.path.exists(xml_path):
        print style.prefix + "Error. The file '%s' does not exist." % xml_path
        sys.exit(style.error_exit_message)
    else:
        print style.prefix + "Validating input XML file '%s'..." % xml_path,
        xmlparse.validate_dtd(xml_path)
        print 'done.'

    # Make sure that the directory to which the temporary aligned images must
    # be copied exists and is writable. Better to check this now than at the
    # last minute after doing all the alignments.
    if options.tempdir:
        msg = "%sA copy of the aligned images will be saved to '%s'"
        print msg % (style.prefix, options.tempdir)
        methods.determine_output_dir(options.tempdir, quiet = True)

    print style.prefix + "Loading input XML file into memory..." ,
    sys.stdout.flush()
    ...
    print "%sMaking sure all the shifted images have the same size... " % \
          style.prefix,  # yes, we are checking this -- see Mosaic.__init__()
    sys.stdout.flush()

    # The context manager of the Mosaic class ensures that all the aligned
    # images are always cleaned up, even in the case of unexpected errors.
    with Mosaic(xml_offsets) as mosaic:

        print 'done.'
        print "%sThere are %d images shifted from '%s'." % \
              (style.prefix, len(mosaic), mosaic.reference.basename)

        # The use of check-images increases the execution time considerably,
        # as SExtractor has to be run on each image in order to generate them.
        if options.check_type:
            print "%sUsing'%s' check-images for the mosaic; this will take remarkably longer." % \
                  (style.prefix, options.check_type)

        print "%sAligning the shifted images with the reference frame... " % style.prefix
        methods.show_progress(0.0)
        for index in range(len(mosaic)):
            mosaic.align(index, checkimage_type = options.check_type)
            methods.show_progress(index / len(mosaic) * 100)
        methods.show_progress(100.0)
        print

        # Although the FITSImage.imcombine() method receives the fraction of
        # highest and lowest pixels to be discarded before combining, let the
        # user know exactly how many pixels are going to be discarded by IRAF's
        # imcombine 'min-max' algorithm
        nimgs = len(mosaic)
        nlow  = int(round(options.flow * nimgs))
        nhigh = int(round(options.fhigh * nimgs))

        print "%sLow pixels to be min-max rejected: %.3f x %d = %d" % \
              (style.prefix, options.flow, nimgs, nlow)
        print "%sHigh pixels to be min-max rejected: %.3f x %d = %d" % \
              (style.prefix, options.fhigh, nimgs, nhigh)

        print "%sMosaicking the %d aligned images into a single FITS image..." % \
              (style.prefix, len(mosaic)) ,
        sys.stdout.flush()

        # As explained earlier, in case the file already exists and has to be
        # overwritten, we do not delete it immediately, but wait right until
        # the aligned images are about to be combined into the new mosaic.
        if os.path.exists(options.output_path):
            os.unlink(options.output_path)

        output_img = mosaic.combine(options.output_path, minimum = options.fraction,
                                    flow = options.flow, fhigh = options.fhigh)
        print 'done.'
        print "%sMosaic image saved to '%s'." % (style.prefix, output_img.path)

        print "%sPixels for which fewer than %.2f of the images were combined " \
              "set to zero." % (style.prefix, options.fraction)

        output_img.update_keyword(options.objectk, options.name)
        print "%sMosaic '%s' FITS keyword updated to '%s'" % \
              (style.prefix, options.objectk, options.name)

        print "%sDetermining the value of the '%s' and '%s' keywords of " \
              "the mosaic..." % (style.prefix, options.rak, options.deck) ,
        sys.stdout.flush()
        ...
        print "%sRight ascension = %f (%.02d %.02d %05.2f)" % ((style.prefix,) + \
              tuple([updated_ra] + list(methods.DD_to_HMS(updated_ra))))
        print "%sDeclination = %f (%+.02d %.02d %05.2f)" % ((style.prefix,) + \
              tuple([updated_dec] + list(methods.DD_to_DMS(updated_dec))))

        # Update the header of the mosaic, so that for future reference it can
        # be checked what exactly this file is and when it was created.

        output_img.add_comment("Mosaic image generated by LEMON")
        output_img.add_comment("LEMON is released under the GPLv3 or later")
        output_img.add_history("Mosaic created on %s" % methods.utctime())

        if options.tempdir:
            assert os.path.exists(options.tempdir)
            assert os.path.isdir(options.tempdir)
            msg = "%sSaving a copy of the temporary aligned images to '%s'..."
            print msg % (style.prefix, options.tempdir) ,
            sys.stdout.flush()
            mosaic.dump(options.tempdir)
            print 'done.'

        # The images removal takes place when the block exits (__exit__ method)
        print "%sAligned images no longer needed, removing them..." % style.prefix ,
        sys.stdout.flush()
