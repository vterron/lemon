...
class XMLOffsetFile(object):
    ...
    def __init__(self, path = None):
        ...
        if not path:
            ...
            self.dom = impl.createDocument(None, 'offsets', None)
            self.root = self.dom.documentElement
        else:
            self.dom = xml.dom.minidom.parse(path)
            self.root = self.dom.getElementsByTagName('offsets')[0]

    def __len__(self):
        """ Return the number of XMLOffsets saved in the XML file """
        return len(self.root.getElementsByTagName('offset'))
    ...
    def add(self, offset):
        ...
        offset_element = self.dom.createElement('offset') # <offset>

        reference_tag = self.dom.createElement('reference')
        reference_value = self.dom.createTextNode(str(offset.reference))
        reference_tag.appendChild(reference_value)
        offset_element.appendChild(reference_tag)

        shifted_tag  = self.dom.createElement('shifted')
        shifted_value = self.dom.createTextNode(str(offset.shifted))
        shifted_tag.appendChild(shifted_value)
        shifted_tag.setAttribute('filter', offset.filter)
        date_str = "%s UTC" % time.asctime(time.gmtime(offset.date))
        shifted_tag.setAttribute('date', date_str)
        offset_element.appendChild(shifted_tag)

        x_tag = self.dom.createElement('x_offset')
        x_value = self.dom.createTextNode(str(offset.x))
        x_tag.appendChild(x_value)
        x_tag.setAttribute('overlap', str(offset.x_overlap))
        offset_element.appendChild(x_tag)

        y_tag = self.dom.createElement('y_offset')
        y_value = self.dom.createTextNode(str(offset.y))
        y_tag.appendChild(y_value)
        y_tag.setAttribute('overlap', str(offset.y_overlap))
        offset_element.appendChild(y_tag)

        # Add an empty line between the previous element (another XMLOffset)
        # in the XML tree and the one we are about to add, indented two tabs.
        newline = self.dom.createTextNode('\n\t\t')
        self.root.appendChild(newline)
        self.root.appendChild(offset_element)   # </offset>

        # Update the 'nimages' attribute of the root element
        self.root.setAttribute('size', str(len(self)))
    ...
    def _toxml(self, encoding = 'utf-8'):
        ...
        header += "<!-- File generated by LEMON on %s UTC -->\n" % \
        ...
        # This approach is more elegant and flexible than having to manually add
        # the trailing newline to each line of the XML Document Type Definition.
        xml_dtd  = '\n'.join(xml_dtd)
        xml_dtd += '\n'

        # toreallyprettyxml() includes a (simplified version of) the XML
        # declaration, but we want to use our own, which also includes the
        # comment stating that the file was generated by LEMON. Therefore,
        # we have to remove the declaration in the first line of the output
        xml_content = \
            toreallyprettyxml(self.dom.toprettyxml(encoding = encoding))
        index = xml_content.find('\n')
        xml_content = xml_content[index+1:]

        return header + xml_dtd + xml_content
    ...
    def __getitem__(self, index):
        ...
        offset = self.root.getElementsByTagName('offset')[index]
        reference = offset.getElementsByTagName('reference')[0].childNodes[0].nodeValue
        shifted   = offset.getElementsByTagName('shifted')[0].childNodes[0].nodeValue
        shifted_filter = offset.getElementsByTagName('shifted')[0].getAttribute('filter')
        pfilter = passband.Passband(shifted_filter)

        shifted_date_str  = offset.getElementsByTagName('shifted')[0].getAttribute('date')
        strptime_format = "%a %b %d %H:%M:%S %Y UTC" # default format + 'UTC'
        # From string to struct_time in UTC to seconds since the epoch
        shifted_date = calendar.timegm(time.strptime(shifted_date_str, strptime_format))

        x         = float(offset.getElementsByTagName('x_offset')[0].childNodes[0].nodeValue)
        x_overlap =   int(offset.getElementsByTagName('x_offset')[0].getAttribute('overlap'))
        y         = float(offset.getElementsByTagName('y_offset')[0].childNodes[0].nodeValue)
        y_overlap =   int(offset.getElementsByTagName('y_offset')[0].getAttribute('overlap'))
        return XMLOffset(reference, shifted, pfilter, shifted_date, x, y, x_overlap, y_overlap)
